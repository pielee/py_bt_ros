...
def deg(d: float) -> float: #각도 변환 함수(degree값을 Radian값으로 변환)
    return math.radians(d)

#2D 평면에서의 회전 각도(Yaw)를 3D 공간의 회전 표현 방식인 쿼터니언으로 변환
#Nav2는 로봇의 자세를 표현할 때 무조건 쿼터니언 형식을 요구하기 때문
#로봇은 평면에서만 움직이므로 'Yaw(좌우 회전)' 값만 있으면 됩니다. 따라서 입력받은 Yaw 각도를 Nav2가 이해할 수 있는 쿼터니언 포맷(z, w)으로 변환하는 수식을 적용함
...
#Nav2 액션 서버에 보낼 목표지점 메시지를 자동으로 만들어주는 함수
def _create_nav_goal(node, x, y, yaw=None, pose_stamped=None):
    goal = NavigateToPose.Goal()
    #pose_stamped 인자가 들어오면, QR 코드에서 읽은 데이터를 그대로 목표로 설정
    if pose_stamped:
        goal.pose = pose_stamped
    #고정 좌표 사용하는 경우 좌표와 각도를 그대로 목표로 설정
    else:
        ps = PoseStamped()
...
class WaitForQRPose(ConditionWithROSTopics): #배달 장소 인식 여부를 판단하는 노드
    ...
    def _predicate(self, agent, blackboard):
        ...
        '''
        행동 트리(MoveToDelivery)가 블랙보드에 있는 좌표 (X=10, Y=20)을 읽으려고 할 때
        X=10을 읽는 그 0.0001초 순간에
        ROS 콜백이 끼어들어서 새로운 좌표 (X=50, Y=90)을 덮어써버림
        행동 트리는 X는 옛날 것(10), Y는 새것(90)을 읽어서 (10, 90)이라는 존재하지 않는 이상한 좌표로 이동해버릴 수 있음
        '''
