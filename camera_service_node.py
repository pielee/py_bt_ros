#!/usr/bin/env python3
"""
ROS2 node: CaptureImageServer

- Subscribes to a camera image topic (sensor_msgs/Image)
- Provides a /capture_image Trigger service
- When called, saves the latest image to a directory as a JPG file
"""
...
class CaptureImageServer(Node):
    def __init__(self):
        ...
        # ---------------- Parameters ----------------
        # camera_topic: which image topic to listen to
        # save_dir: where to save captured images
        ...
        self.get_logger().info(
            ...
            f"Listening on '{self.camera_topic}', saving to '{self.save_dir}'"
        )
    ...
    def capture_callback(self, request, response):
        ...
        if self.last_image is None:
            ...
            self.get_logger().warn(
                "[CaptureImageServer] No image available yet."
            )
            return response
        ...
        try:
            ...
            self.get_logger().error(
                f"[CaptureImageServer] cv_bridge error: {e}"
            )
            return response
        ...
        try:
            ...
            self.get_logger().error(
                f"[CaptureImageServer] Save error: {e}"
            )
            return response
        ...
        self.get_logger().info(
            f"[CaptureImageServer] Saved image → {filename}"
        )
        return response
